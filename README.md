...
![](/Users/linshengfeng/学习/机器学习/强化学习/多臂老虎机/概率分布图像.jpg)
...
$$
Q(a)= {\Sigma~Reward_a \over len(Reward_a)},a是老虎机序号
$$
也可以用另一个思路理解: ```t```时刻某老虎机```a```的```Q值```,是之前选择该老虎机获得的总收益除以选择它的次数,公式中```1(Ai = a)```表示```i```时刻的动作(也就是选择的老虎机)如果是```a```则标记为1,否则为0.
$$
Q_t(a)= {\Sigma_{i=1}^{t-1}R_i*1(A_i=a) \over \Sigma_{i=1}^{t-1}1(A_i=a)},a是老虎机序号
$$
其计算代码如下:
